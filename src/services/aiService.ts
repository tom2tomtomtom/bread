...
const generateImagePrompt = (
  ...
  const sceneryIndex =
    Math.abs(headline.text.length + territory.title.length) % sceneryOptions.length;
...
const generateFallbackPrompt = (_headline: { text: string; followUp: string }): string => {
...
const generateSingleImage = async (
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      console.log(
        `🎨 Generating image for Territory ${territoryIndex + 1}, Headline ${headlineIndex + 1} (Attempt ${attempt}/${maxRetries})`
      );
      ...
      if (imageUrl) {
        console.log(
          `✅ Image generated successfully for Territory ${territoryIndex + 1}, Headline ${headlineIndex + 1} on attempt ${attempt}`
        );
        return {
          territoryIndex,
          headlineIndex,
      ...
      lastError = error;
      console.error(
        `❌ Attempt ${attempt} failed for Territory ${territoryIndex + 1}, Headline ${headlineIndex + 1}:`,
        error.message
      );
  ...
  // All attempts failed
  console.error(
    `💥 All ${maxRetries} attempts failed for Territory ${territoryIndex + 1}, Headline ${headlineIndex + 1}:`,
    lastError?.message
  );
  return {
    territoryIndex,
...
const generateHeadlineImages = async (
  territories: any[],
  brief: string,
  openai: OpenAI
): Promise<any[]> => {
  ...
  for (let i = 0; i < allImageRequests.length; i += batchSize) {
    ...
    console.log(
      `🎨 Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(allImageRequests.length / batchSize)}...`
    );
  ...
  if (failCount > 0) {
    console.log(
      ...
      results
        .filter(r => !r.imageUrl)
        .map(r => `Territory ${r.territoryIndex + 1}, Headline ${r.headlineIndex + 1}: ${r.error}`)
    );
  }
...
const mockResponse = (): GeneratedOutput => ({
  territories: [
    {
      ...
      headlines: [
        ...
        },
        {
          ...
          reasoning:
            'Direct challenge to sale-focused retailers, emphasizing consistent value over sporadic discounts.',
          confidence: 78,
        },
        {
          ...
          reasoning:
            'Elevates mundane shopping experience while maintaining approachable, everyday positioning.',
          confidence: 82,
        },
      ],
    },
    {
      ...
      positioning:
        'Everyday Rewards turns every shop into a win for savvy Australians who value consistency over chaos.',
      ...
      headlines: [
        {
          ...
          reasoning:
            'Creates exclusivity while emphasizing the value of regular shopping behavior.',
          confidence: 80,
        },
        {
    ...
    },
    {
      ...
      title: 'No FOMO Zone',
      positioning:
        'Skip the sales pressure and enjoy consistent rewards without the rush, stress, or limited-time anxiety.',
      tone: 'Calm, anti-FOMO',
      headlines: [
        {
          ...
          reasoning:
            'Directly counters urgency-driven sales tactics, promoting stress-free shopping.',
          confidence: 88,
        },
        {
    ...
    },
    {
      ...
      positioning:
        'Celebrating Australian shoppers with rewards that reflect our values of fairness and community.',
    ...
    },
    {
      ...
      title: 'Steady Wins',
      positioning:
        'Smart members quietly accumulate genuine value while others chase one-off deals and flash sales.',
      tone: 'Wry, confident',
      headlines: [
        {
    ...
    },
    {
      ...
      positioning:
        'Every day is the perfect day to earn rewards with Everyday Rewards - no special occasion required.',
  ...
  compliance: {
    ...
    output:
      'All messaging complies with Australian Consumer Law and ACCC advertising guidelines. Claims are substantiated with program benefits and terms are clearly disclosed where required.',
    notes: [
...
const parseAIResponse = (text: string): GeneratedOutput => {
  ...
  try {
    ...
    throw new Error(
      `Invalid structure from OpenAI. Got: ${JSON.stringify({
        territories: parsed.territories ? `${parsed.territories.length} items` : 'missing',
        compliance: parsed.compliance ? 'present' : 'missing',
        keys: Object.keys(parsed),
      })}`
    ...
    throw new Error(
      `Failed to parse OpenAI response: ${errorMessage}. Raw response: ${text.substring(0, 200)}...`
    );
  }
};
...
- confidence: Numerical score 1-100 based on market fit and effectiveness`,
    ...
    if (generateImages && parsed.territories) {
      ...
      try {
        const imageResults = await generateHeadlineImages(
          parsed.territories,
          brief || prompt,
          openai
        );
